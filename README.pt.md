...
Sincronização labial em tempo real para avatares VRM, expressões faciais, animações de repouso e texto para fala (TTS) – tudo construído com Godot 4.3 ou superior, e utilizando uma ponte Node.js para a síntese de voz.

## O que isso demonstra

1. **Microfone de entrada -> o rosto se move de forma convincente** a 60 quadros por segundo, com visemes de FFT sem latência.
2. **Webcam de entrada -> rastreamento completo do rosto** através do OpenSeeFace (52 blendshapes do ARKit).
...
| Sincronização labial com base em visemas, utilizando a Transformada Rápida de Fourier (FFT) (microfone/WAV). | Trabalhando. |
...
| Biblioteca de avatares (navegue e faça o download de modelos VRM com licença CC0). | Trabalhando. |
...
- **Driver da webcam:** [OpenSeeFace](https://github.com/emilianavt/OpenSeeFace) via UDP (52 formas de deformação ARKit + pose da cabeça).
...
- **Motor de TTS:** KokoroSharp (local, executa na GPU ou CPU).
...
Um arquivo de teste localizado em `assets/audio/test_vowels.wav` percorre todas as cinco bandas de visemas (ou, oh, aa, ih, ee) duas vezes, durante aproximadamente 10 segundos. Clique em "Reproduzir Sons de Teste" para verificar se o driver FFT está funcionando corretamente.
...
2. Na interface de demonstração, altere a opção "Driver" para **OpenSeeFace (Webcam)**.
...
1. Certifique-se de que o `voice-soundboard-mcp` está em execução (consulte o repositório para instruções de configuração).
...
4. As vozes disponíveis são carregadas a partir do servidor (padrão: `am_fenrir`).
5. Opcional: selecione uma emoção para aplicar dicas de expressão durante a fala.

Se a conexão automática da ponte não funcionar, utilize o botão "Conectar" manualmente no painel de controle do sistema de texto para fala (TTS).
...
| **Model Diagnostics** | Ativar/desativar o painel de diagnóstico. |
| **Avatar Library** | Explore e faça o download de avatares VRM com licença CC0. |
...
- **Status:** VERDE (tudo mapeado), AMARELO (mapeamento parcial), VERMELHO (falta mapear formas críticas).
...
- **Status da piscada e do osso ocular:** Indica se a piscada procedural e o olhar baseado em ossos funcionarão.
...
| `smoothing.release_time` | A velocidade com que os pesos caem (em segundos). | 0.12 |
...
| `blink.*` | Temporização dos piscadelas (intervalo, duração, probabilidade de piscadelas duplas). | Consulte o arquivo. |
...
| Please provide the English text you would like me to translate. I am ready to translate it into Portuguese. | "Please provide the text you would like me to translate." | "The company is committed to providing high-quality products and services."

-----------------
"A empresa está comprometida em fornecer produtos e serviços de alta qualidade." |
...
- **Todos os dicionários de "hot path" são pré-alocados** – pressão de coleta de lixo (GC) nula por quadro.
...
- **Posição "T" dos braços:** Os modelos VRM são carregados na posição "T" após a reatribuição. O script `pose_corrector.gd` tenta corrigir a posição em tempo real usando `set_bone_pose_rotation()`, mas a matemática dos eixos locais das articulações não está produzindo resultados corretos. Este é o problema visual mais significativo. A correção correta provavelmente requer ou a modificação da pose de referência para a reatribuição no processo de importação do VRM, ou a solução empírica dos eixos de rotação locais das articulações para cada esqueleto.
- **Modelos para VRChat:** Os nomes das formas de deformação (blend shapes) usam o prefixo `blendShape1.vrc_v_*`. O perfil de mapeamento do VRChat lida com isso, mas a detecção automática pode sugerir o perfil errado em alguns modelos.
...
- [ ] **Atalhos de teclado:** Atalhos para ações comuns (ativar/desativar o microfone, alternar de avatar, acionar expressões).
- [ ] **Integração com OBS:** Modo de fundo transparente e saída de câmera virtual para transmissões ao vivo.
- [ ] **Suporte para múltiplas vozes:** Atribuição de vozes diferentes para cada avatar.
- [ ] **Variação de animações de inatividade aprimorada:** Animações de inatividade aleatórias para evitar repetições robóticas.
